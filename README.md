...
```env
# ===========================================
# フロントエンド設定（EC2等リモートサーバーへのデプロイ時に必須）
# ===========================================
# VITE_API_URL=http://<EC2のIPアドレスまたはドメイン>:5000
...
#### フロントエンド設定（VITE_API_URL）

EC2等のリモートサーバーにデプロイする場合、フロントエンドがバックエンドAPIにアクセスするURLを設定する必要があります。
...
| 設定 | デフォルト | 説明 |
|------|-----------|------|
| `VITE_API_URL` | `http://localhost:5000` | バックエンドAPIのURL |

**重要**: `VITE_*`環境変数は**ビルド時**にJavaScriptに埋め込まれます。

```bash
# 正しい手順
# 1. .envにVITE_API_URLを設定
# 2. npm run build でビルド（この時点で値が埋め込まれる）
# 3. dist/フォルダをデプロイ
```

**ローカル開発時**: `VITE_API_URL`を設定しなければ、自動的に`http://localhost:5000`にアクセスします。
...
1. **重みは相対比率として正規化されます**
   - 合計が100である必要はありません
   - 例: `40, 30, 30` と `80, 60, 60` は同じ比率（4:3:3）として扱われます
   - 設定した重みの合計で各値を割り、ブースト係数として使用します

2. **テキスト検索とベクトル検索は自由に組み合わせ可能です**
   ```env
   # 例: テキスト検索とベクトル検索のハイブリッド
   SEARCH_PROPER_NOUNS_TEXT_WEIGHT=40    # 固有名詞の完全一致検索
   SEARCH_TAGS_VECTOR_WEIGHT=40          # タグのベクトル検索
   SEARCH_ABSTRACT_VECTOR_WEIGHT=20      # 要約のベクトル検索
   # → 4:4:2 の比率で3つの検索を組み合わせ
   ```

3. **重みが0の検索方法は完全に無効化されます**
   - 不要な検索をスキップすることでパフォーマンスが向上します
...
手軽に起動できますが、本番運用には推奨しません。

```bash
# VITE_API_URLは.envから自動的に読み込まれる
