          ...
          <div className="flex items-center justify-between px-4 py-2.5 bg-muted/30 border-t border-border">
            <div className="flex items-center gap-2">
              <Select value={mode} onValueChange={(value) => onModeChange(value as "search" | "assistant")}>
                <SelectTrigger className="w-[140px] h-8 bg-background border-border hover:bg-accent transition-colors">
                  <SelectValue>
                    <div className="flex items-center gap-2">
                      {mode === "search" ? (
                        <>
              ...
              <span className="text-xs text-muted-foreground">で実行</span>
            </div>
          </div>
        </div>
      </div>
