...
【重要：検索用要約のルール】
この要約はベクトル検索でヒットさせるために使用されます。
以下の情報は特に重要なため、**要約の冒頭部分に優先的に記載**してください：

1. **人名と役割**（最優先）
   - 決済者、承認者、報告者、担当者、責任者などの役割付き氏名
   - 例：「決済者：山田太郎、報告者：鈴木一郎」
...
3. **プロジェクト・委員会情報**
   - プロジェクト名（PJ名）、委員会名、ワーキンググループ名
   - 例：「次世代電池開発PJ」「材料評価委員会」

4. **研究・技術内容**
   - 上記の情報を記載した後、本文の内容を要約

【出力形式】
要約は日本語で、{max_length}文字以内にしてください。
冒頭に人名・組織名・PJ名を記載し、その後に内容の要約を続けてください。"""

        prompt = f"""以下のテキストを要約してください。
人名、役割、会社名、部署名、プロジェクト名は要約の冒頭に記載してください。
...
【最重要：冒頭に記載すべき情報】
この要約はベクトル検索でヒットさせるために使用されます。
以下の情報は**必ず要約の冒頭（最初の2〜3文）に記載**してください：

■ 人名と役割
- 決済者、承認者、報告者、担当者、責任者、リーダーなどの役割付き氏名
- 研究者、開発者の氏名
- 例：「本研究の責任者は山田太郎、報告者は鈴木一郎である。」
...
【Abstract作成手順】
冒頭に上記の情報を記載した後、以下の6つの観点で内容を要約してください。
...
【出力ルール】
- **冒頭に人名・組織名・PJ名を必ず記載**（検索でヒットさせるため）
...
【重要】要約の冒頭に以下を必ず記載してください：
- 関係者の氏名と役割（決済者、報告者、担当者など）
- 組織名（会社名、部署名）
- プロジェクト名・委員会名

その後、Background, Objective, Method, Result, Discussion, Future Planの各観点を網羅し、
...
【最重要：冒頭に記載すべき情報】
以下の情報がデータ内にあれば、**必ず要約の冒頭に記載**してください：
...
【出力ルール】
- **冒頭に人名・組織名・PJ名があれば必ず記載**
- 日本語で{max_length}文字以内
- 箇条書きではなく、自然な文章として出力
- 具体的な数値や固有名詞を含める"""

        prompt = f"""以下の表データを分析し、検索可能な要約を作成してください。
人名、組織名、プロジェクト名がデータ内にあれば、要約の冒頭に記載してください。
...
【抽出ルール】
ファイルパス、フォルダ名、ファイル名のどこにあっても、固有名詞は**すべて抽出**してください。
位置による優先度はつけず、見つけた固有名詞はすべて出力してください。

【抽出対象】
1. 製品識別番号・型番
   - 短い英数字コード（2〜5文字程度）: 6ST, 3TX, 3US, AD4DI, 6S9, AP4DI
   - ハイフン付きコード: M3X-500, X-100A, RD-2024-001
   - 数字+英字: 6ST, 3TX, 6S9
   - 英字+数字+英字: AD4DI, AP4DI
   - 英字+数字: AB12, X100

2. 研究ID
   - 例: RD-2024-001, 研究-A-123, PROJ-001

3. 素材識別子
   - 例: CFRP, Al6061, SUS304, アルミ合金, カーボン

4. その他の識別子
   - 機種名、部品番号、規格番号など

【除外対象】
- 一般的な単語（report, document, 資料, データ, 結果, 報告書, 評価, 試験など）
- 日付のみ（2024, 202401, 2024-01など）
- 拡張子（.pdf, .docx, .xlsxなど）
- 一般的なフォルダ名（backup, archive, old, new, temp, workなど）

出力例: ["6ST", "3TX", "CFRP", "AD4DI", "RD-2024-001"]
固有名詞がない場合: []"""

        prompt = f"""以下のファイルパスとファイル名から、固有名詞をすべて抽出してJSON配列で出力してください。

【ルール】
- パス内のどこにあっても固有名詞はすべて抽出
- 短い英数字コード（6ST, 3TX, AD4DIなど）も重要な固有名詞
- 可能な限り多く抽出してください
