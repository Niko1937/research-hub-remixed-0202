...
【最重要：製品コード・型番の抽出】
以下のような短い英数字コード（2〜5文字程度）は製品識別番号として非常に重要です。
必ず抽出してください：
- 数字+英字: 6ST, 3TX, 3US, 6S9
- 英字+数字+英字: AD4DI, AP4DI, M3X
- 英字+数字: AB12, X100

抽出対象:
1. 製品識別番号・型番（最重要）
   - 短い英数字コード: 6ST, 3TX, 3US, AD4DI, 6S9, AP4DI, M3X-500, X-100A
   - フォルダ名やファイル名の先頭にある英数字コードは製品コードの可能性が高い

2. 研究ID
   - 例: RD-2024-001, 研究-A-123, PROJ-001, AB12
...
- 日付のみ（2024, 202401, 2024-01など）※製品コードと組み合わさっている場合は抽出
- 拡張子（.pdf, .docx, .xlsxなど）
- 一般的なフォルダ名（backup, archive, old, new, tempなど）

出力例: ["6ST", "3TX", "CFRP", "RD-2024-001"]
固有名詞がない場合: []"""

        prompt = f"""以下のファイルパスとファイル名から、固有名詞（特に製品コード・型番）を抽出してJSON配列で出力してください。

【特に注意】
- フォルダ名やファイル名の先頭にある「6ST」「3TX」「AD4DI」のような短い英数字コードは製品識別番号です
- これらは非常に重要なので必ず抽出してください
